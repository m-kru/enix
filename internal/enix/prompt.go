...
func (p *prompt) Render() {
	...
	p.Cursor.Render(frame.Line(1, 0), p.View, cfg.Style.Cursor)
...
func (p *prompt) openPathMenu(path string) {
	...
	PromptMenu = newMenu(&PromptFrame, names, 0, cfg.Style.Menu, cfg.Style.MenuItem)
...
func (p *prompt) Exec() TcellEventReceiver {
	...
	for range c.RepCount {
		...
		case "sel-prev-line":
			err = exec.SelPrevLine(c.Args, tab)
		case "sp", "sel-paren":
